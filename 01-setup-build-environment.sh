...
mock -r fedora-32-x86_64 --chain ioprocess-1.4.1-1.fc32.src.rpm vdsm-4.40.22-1.fc32.src.rpm ovirt-hosted-engine-ha-2.4.4-1.fc32.src.rpm
  626  sudo find /var/lib/mock/fedora-32-x86_64/root/ | grep noarch'\.'rpm$ | awk '{print "mv "$1" /var/repo/noarch"}' > mv-noarch.sh
  627  sudo bash -x mv-noarch.sh
mock -r fedora-32-x86_64 ovirt-hosted-engine-setup-2.4.5-1.fc32.src.rpm
  631  sudo find /var/lib/mock/fedora-32-x86_64/root/ | grep noarch'\.'rpm$ | awk '{print "mv "$1" /var/repo/noarch"}' > mv-noarch.sh
  632  sudo bash -x mv-noarch.sh
...
mock -r fedora-32-x86_64 --clean && mock -r fedora-32-x86_64 --init
 ...
 mock -r fedora-32-x86_64 ovirt-log-collector-4.4.3-1.fc32.src.rpm
  652  sudo find /var/lib/mock/fedora-32-x86_64/root/ | grep noarch'\.'rpm$ | awk '{print "mv "$1" /var/repo/noarch"}' > mv-noarch.sh
  653  sudo bash -x mv-noarch.sh
mock -r fedora-32-x86_64 ovirt-node-ng-4.4.0-1.fc32.src.rpm
  661  sudo find /var/lib/mock/fedora-32-x86_64/root/ | grep noarch'\.'rpm$ | awk '{print "mv "$1" /var/repo/noarch"}' > mv-noarch.sh
  662  sudo bash -x mv-noarch.sh
 mock -r fedora-32-x86_64 ovirt-provider-ovn-1.2.30-1.fc32.src.rpm
  666  sudo find /var/lib/mock/fedora-32-x86_64/root/ | grep noarch'\.'rpm$ | awk '{print "mv "$1" /var/repo/noarch"}' > mv-noarch.sh
  667  sudo bash -x mv-noarch.sh
mock -r fedora-32-x86_64 ovirt-release44-4.4.1.5-1.fc32.src.rpm
  674  sudo find /var/lib/mock/fedora-32-x86_64/root/ | grep noarch'\.'rpm$ | awk '{print "mv "$1" /var/repo/noarch"}' > mv-noarch.sh
  675  sudo bash -x mv-noarch.sh
